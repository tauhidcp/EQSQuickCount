...
type
  ...
  TFGrafikPartai = class(TForm)
    BCetak: TBitBtn;
    BRefresh: TBitBtn;
    BTutup: TBitBtn;
    CkAutoRefresh: TCheckBox;
    GrafikPartai: TChart;
    GrafikPartaiBarSeries2: TBarSeries;
    ...
    procedure GrafikPartaiAfterDraw(ASender: TChart; ADrawer: IChartDrawer);
...
begin
 Result := 'SELECT sum('+Total+') as jml FROM '+
           '((((((t_hitungpartai INNER JOIN t_partai ON t_partai.no_urut=t_hitungpartai.no_urut) '+
...
procedure TFGrafikPartai.BTutupClick(Sender: TObject);
begin
...
procedure TFGrafikPartai.BRefreshClick(Sender: TObject);
begin
  HapusGrafik;
  BuatGrafik;
  Persentase;
...
procedure TFGrafikPartai.BCetakClick(Sender: TObject);
var
  namaimg,titel : string;
begin
  namaimg:='gpartai.bmp';
  titel  :='Grafik Perhitungan Partai';
  if FUtama.SimpanGrafik.Execute then
     begin
     GrafikPartai.SaveToBitmapFile(ExtractFilePath(Application.ExeName)+'tmp/'+namaimg);
...
procedure TFGrafikPartai.CkAutoRefreshChange(Sender: TObject);
begin
...
procedure TFGrafikPartai.FormCreate(Sender: TObject);
begin
  Bars := TBarSeries.Create(GrafikPartai);
  Bars.Marks.Style := TSeriesMarksStyle(smsValue);
  Bars.Marks.Distance:= 125;
end;

procedure TFGrafikPartai.FormResize(Sender: TObject);
begin
  UkuranPanel;
end;

procedure TFGrafikPartai.FormShow(Sender: TObject);
begin
    jml := DM.ZQPartai.RecordCount;
end;

procedure TFGrafikPartai.GrafikPartaiAfterDraw(ASender: TChart;
...
var
  ...
  jpg: TJpegImage;
begin
    if (DM.ZQPartai.RecordCount>0) then begin
  DM.ZQPartai.First;

  for i:=0 to GrafikPartai.BottomAxis.ValueCount-1 do begin
    bmp := TBitmap.Create;
  jpg := TJpegImage.Create;
    item := GrafikPartai.BottomAxis.Value[i];
    xgr := GrafikPartaiBarSeries2.AxisToGraphX(item.FValue);
    ximg := GrafikPartai.XGraphToImage(xgr);
    ...
    jpg.LoadfromFile(ExtractFilePath(Application.ExeName)+'img\partai\'+DM.ZQPartai.FieldByName('gambar').AsString) else
    jpg.LoadfromFile(ExtractFilePath(Application.ExeName)+'img\noimage.jpg');

    bmp.Assign(jpg);

    if (GrafikPartai.BottomAxis.ValueCount<=2) then
    begin
    p := Bars.GetBarWidth(i) div 4;
    l := Bars.GetBarWidth(i) div 4;
    end else
    if (GrafikPartai.BottomAxis.ValueCount<5) and (GrafikPartai.BottomAxis.ValueCount>2) then
    begin
    p := Bars.GetBarWidth(i) div 3;
    l := Bars.GetBarWidth(i) div 3;
    end else
    if (GrafikPartai.BottomAxis.ValueCount>=5) then
    begin
...
procedure TFGrafikPartai.TimerRefreshTimer(Sender: TObject);
begin
  HapusGrafik;
  BuatGrafik;
  Persentase;
...
procedure TFGrafikPartai.BuatGrafik;
var
i : integer;
persen : Real;
ket : string;
begin
  ...
  if (DM.ZQPartai.RecordCount>0) then begin
  DM.ZQPartai.First;
  for i := 0 to DM.ZQPartai.RecordCount-1 do
  begin
  with DM.ZQGrafikPartai do
  begin
  Close;
  SQL.Clear;
  SQL.Text:=BuatQuery('t_hitungpartai.'+ket)+' where t_partai.no_urut="'+DM.ZQPartai.FieldByName('no_urut').AsString+'"';
  Open;
  end;
  if (NilaiUnik('t_hitungpartai','suara_sah') > 0) and (DM.ZQGrafikPartai.FieldByName('jml').AsInteger > 0) then
  persen := StrToFloat(FormatFloat('0.##',(DM.ZQGrafikPartai.FieldByName('jml').AsInteger/NilaiUnik('t_hitungpartai','suara_sah'))*100)) else
  persen := 0;
  Bars.AddXY(DM.ZQPartai.FieldByName('no_urut').AsInteger,
             persen,
             DM.ZQPartai.FieldByName('singkatan').AsString+sLineBreak+'('+
             FormatCurr('#,###',DM.ZQGrafikPartai.FieldByName('jml').AsCurrency)
             +')',
             StringToColor(DM.ZQPartai.FieldByName('warna').AsString)
             );
  DM.ZQPartai.Next;
  end;
  GrafikPartai.AddSeries(Bars);
  GrafikPartai.AxisList.BottomAxis.Marks.Source := Bars.Source;
  GrafikPartai.AxisList.BottomAxis.Marks.Style := smsLabel;
  end;
end;

procedure TFGrafikPartai.HapusGrafik;
var
i : integer;
begin
...
procedure TFGrafikPartai.Persentase;
var
...
begin
  ...
  begin
  // Suara Sah
  PSuaraSah.Caption:='Suara Sah : '+FormatCurr('#,###',NilaiUnik('t_hitungpartai','suara_sah'));
  // Suara Tidak Sah
  PSuaraTidakSah.Caption:='Suara Tidak Sah : '+FormatCurr('#,###',NilaiUnik('t_hitungpartai','suara_tidaksah'));
  // TPS Masuk
  Close;
  SQL.Clear;
  SQL.Text:='select distinct(tps) as jml from t_hitungpartai';
  Open;
  PTPSMasuk.Caption:='TPS Masuk : '+FormatCurr('#,###',RecordCount);
  // Suara Masuk
  PSuaraMasuk.Caption:='Suara Masuk : '+FormatCurr('#,###',NilaiUnik('t_hitungpartai','dpt'));
  ...
  Open;
  PJmlPemilih.Caption:='Jumlah Pemilih : '+FormatCurr('#,###',FieldByName('jml').AsInteger);
...
procedure TFGrafikPartai.UkuranPanel;
begin
...
function TFGrafikPartai.NilaiUnik(NamaTabel:String;NamaField:String):LongInt;
var
  i,hasil : LongInt;
begin
  Result:=0;
