...
uses
  ...
  Buttons, VirtualTrees, rxdbgrid, udatamodule, uubahhitungdpdri,
...
type
  ...
  TFHitungDPDRI = class(TForm)
    ...
    procedure GridHitungDPDPrepareCanvas(sender: TObject; DataCol: Integer;
...
var
  FHitungDPDRI: TFHitungDPDRI;
  i,j,k,l,m,n : integer;

const
  query =   'SELECT t_calegdpdri.no_urut, t_calegdpdri.nama_caleg, t_provinsi.nama_provinsi, '+
            ...
            'ORDER BY t_calegdpdri.no_urut ASC';
...
// getNoUrut Caleg
function getNoUrut(nama: string;dapil:string): string;
begin
 Result:='';
 with DM.ZQCari do
 begin
  Close;
  SQL.Clear;
  SQL.Text:='select no_urut as id from t_calegdpdri where nama_caleg="'+nama+'" and dapil="'+getIdProvinsi(dapil)+'"';
  Open;
 end;
   ...
   AQuery.Close;
     ...
     for y := 1 to MaxRow do
      ...
      utama.PBLoading.Position:=y;
          ...
          begin
          AQuery.SQL.Clear;
          AQuery.SQL.Text:='insert into t_hitungdpdri (dapil,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values '+
          '("'+getIdProvinsi(XLApp.Cells[y,3].Value)+'","'+
          getNoUrut(XLApp.Cells[y,2].Value,XLApp.Cells[y,3].Value)+'","'+getIdTPS(XLApp.Cells[y,5].Value,XLApp.Cells[y,4].Value)+'","'+Trim(XLApp.Cells[y,6].Value)
          +'","'+Trim(XLApp.Cells[y,7].Value)+'","'+Trim(XLApp.Cells[y,8].Value)+'","'+Trim(XLApp.Cells[y,9].Value)
          +'")';
          AQuery.ExecSQL;
          end else
          begin
          if (cekData('t_hitungdpdri',10)=True) then exit else
             begin
             AQuery.SQL.Clear;
             AQuery.SQL.Text:='insert into t_hitungdpdri (dapil,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values '+
             '("'+getIdProvinsi(XLApp.Cells[y,3].Value)+'","'+
...
procedure TFHitungDPDRI.BTutupClick(Sender: TObject);
begin
  FUtama.ImageBack.Visible:=True;
  Close;
end;

procedure TFHitungDPDRI.GridHitungDPDPrepareCanvas(sender: TObject;
  DataCol: Integer; Column: TColumn; AState: TGridDrawState);
begin
        with TStringGrid(GridHitungDPD) do
  begin
      Options := Options + [goRowSelect];
  end;
end;

function TFHitungDPDRI.SaveAsExcelFile(AQuery: TZQuery; AFileName: string): Boolean;
var
...
try
  ...
  MyWorksheet := MyWorkbook.AddWorksheet('Data_Hitung_DPD');
  AQuery.First;
  ...
  MyWorksheet.WriteCellValueAsString(i, 1, AQuery.FieldByName('nama_caleg').AsString);
...
procedure TFHitungDPDRI.BEditClick(Sender: TObject);
begin
  with FUbahHitungDPDRI do
  begin
    Caption:='Ubah Data Perhitungan DPD RI';
    hapus;
    ShowModal;
  end;
end;

procedure TFHitungDPDRI.BEksportClick(Sender: TObject);
begin
 if FUtama.SaveExcel.Execute then
    begin
    SaveAsExcelFile(DM.ZQHDPD,FUtama.SaveExcel.FileName);
    MessageDlg('Data Hasil Perhitungan DPD RI Berhasil Dieksport Ke '+FUtama.SaveExcel.FileName,mtInformation,[mbok],0);
    end;
end;

procedure TFHitungDPDRI.BFilterClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
    CBKat.Items.Add(DM.ZQDPDRI.FieldByName('nama_caleg').AsString);
    DM.ZQDPDRI.Next;
    end;
    // End Set
    ShowModal;
...
procedure TFHitungDPDRI.BImportClick(Sender: TObject);
begin
  if FUtama.ExcelDialog.Execute then
  begin
  ReadXLSFile(DM.ZQCari5,FUtama.ExcelDialog.FileName,FUtama);
  MessageDlg('Data Perhitungan DPD RI Berhasil Diimport!',mtInformation,[mbok],0);
  end;
end;

procedure TFHitungDPDRI.BKosongkanClick(Sender: TObject);
begin
  if MessageDlg('Anda Akan Menghapus SEMUA Data Perhitungan CALEG DPD RI?',mtConfirmation,[mbyes,mbno],0)=mryes then
  begin
  // Hapus Database
  with DM.ZQCari do
    begin
      Close;
      SQL.Clear;
      SQL.Text:='truncate table t_hitungdpdri';
      ExecSQL;
    end;
  BRefresh.Click;
  end;
...
procedure TFHitungDPDRI.BRefreshClick(Sender: TObject);
begin
  with DM.ZQHDPD do
  begin
  Close;
