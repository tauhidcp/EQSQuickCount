...
function cekSaksi(idtelegram:string):integer;
function cekKoor(idtelegram:string):integer;
function cekPolling(idtelegram: string): integer;
function cekRelawan(nohp:string;idtelegram:string):integer;
function cekRelawanAda(idtelegram:string):integer;
function cekUsulanAda(idtelegram:string):integer;
...
procedure kirimChat(idchat:string;idtelegram:string;isi:string);
function getIdTPSSaksi(idtelegram:string):string;
...
pesan, pemisah, idtelegram, tglchat, ssah, stsah, dpt, nmkel, notps, tglpemilu, idchat, aksi, hasil, hasil1, hasil2, hasil3, hasil4, hasil5, hasil6, hasil7, hasil8, hasil9 : string;
begin
  ...
  with DM.ZQCariChat do begin
   Close;
   SQL.Clear;
   SQL.Text:='select * from t_inboxchat where processed="False" order by id asc limit 1';
   Open;
    ...
    // for i := 1 to RecordCount do begin
     ...
     idtelegram  := FieldByName('id_pengirim').AsString;
     idchat      := FieldByName('id_chat').AsString;
     tglchat     := FieldByName('tanggal').AsString;
     kar_x := posex(' ',pesan,1);
     aksi  := Trim(AnsiUpperCase(copy(pesan,1,kar_x)));
         ...
         if (aksi='DAFTAR') then
            ...
            hasil4   := copy(pesan,kar_awal+1);
              ...
              begin
                ...
                kirimChat(idchat,idtelegram,getBalasan(1))
                else begin
                // Data Benar
                if cekSaksiD(hasil5,idtelegram,getIdTPS(hasil4,hasil3))<=0 then
                begin
                with DM.ZQSimpan do
                begin
                  Close;
                  SQL.Clear;
                  SQL.Text:='insert into t_saksi (nama_saksi,desa_kelurahan,tps,nohp,id_telegram) values ("'+hasil2+'","'+getIdKelurahan(hasil3)+'","'+getIdTPS(hasil4,hasil3)+'","'+ReplaceNol(hasil5)+'","'+idtelegram+'")';
                  ExecSQL;
                end;
                // Saksi Tersimpan
                kirimChat(idchat,idtelegram,getBalasan(2));
                end else
                // Nomor Sudah digunakan
                kirimChat(idchat,idtelegram,getBalasan(3));
                end;
              end else
              if (hasil1='RELAWAN') then
              begin
                if (getIdKelurahan(hasil3)='') then
                kirimChat(idchat,idtelegram,getBalasan(4))
                else begin
                if cekRelawan(hasil4,idtelegram)<=0 then begin
                // Data Benar
                with DM.ZQSimpan do
                begin
                  Close;
                  SQL.Clear;
                  SQL.Text:='insert into t_timses (nama_timses,desa_kelurahan,nohp,id_telegram) values ("'+hasil2+'","'+getIdKelurahan(hasil3)+'","'+ReplaceNol(hasil4)+'","'+idtelegram+'")';
                  ExecSQL;
                end;
                // Relawan Tersimpan
                kirimChat(idchat,idtelegram,getBalasan(5));
                end else
                // Nomor Sudah digunakan
                kirimChat(idchat,idtelegram,getBalasan(6));
                end;
              end else
              // KOORTPS
              if (hasil1='KOORTPS') then
                 ...
                 if (getIdKelurahan(hasil3)='') then
                     ...
                     // Data Benar
                     if (cekUsulanAda(idtelegram)<=0) then
                     begin
                     with DM.ZQSimpan do
                     begin
                       Close;
                       SQL.Clear;
                       SQL.Text:='insert into t_usulantimses (nama,desa_kelurahan,nohp,idchat,idtelegram) values ("'+hasil2+'","'+getIdKelurahan(hasil3)+'","'+hasil4+'","'+idchat+'","'+idtelegram+'")';
                       ExecSQL;
                     end;
                     // Usulan Relawan Tersimpan
                     kirimChat(idchat,idtelegram,getBalasan(7));
                     end else
                     kirimChat(idchat,idtelegram,getBalasan(8));
                     end;
                 end;  // End Simpatisan
         ...
         if (aksi='DUKUNG') then
            ...
            if (getIdKelurahan(hasil3)='') then
                ...
                begin
                  Close;
                  SQL.Clear;
                  SQL.Text:='insert into t_dukunganindie (noktp,nama,desa_kelurahan,nohp,idtelegram,idchat) values ("'+hasil1+'","'+hasil2+'","'+getIdKelurahan(hasil3)+'","-","'+idtelegram+'","'+idchat+'")';
                  ExecSQL;
                end;
                // Dukungan Independen Tersimpan
                kirimChat(idchat,idtelegram,getBalasan(9));
                end else
                // KTP Sudah Terdaftar
                kirimChat(idchat,idtelegram,getBalasan(10));
                end;
         ...
         if (aksi='PILIH') then
            ...
            if (getIdKelurahan(hasil2)='') then
                ...
                // Data Benar
                if (cekPolling(idtelegram)<=0) then
                begin
                with DM.ZQSimpan do
                begin
                  Close;
                  SQL.Clear;
                  SQL.Text:='insert into t_polling (nama,desa_kelurahan,nohp,idtelegram,idchat,pekerjaan) values ("'+hasil1+'","'+getIdKelurahan(hasil2)+'","-","'+idtelegram+'","'+idchat+'","'+hasil3+'")';
                  ExecSQL;
                end;
                // Polling Tersimpan
                kirimChat(idchat,idtelegram,getBalasan(9));
	    ...
	    kar_awal := posex(pemisah,pesan,kar_akhir);
            ...
            if (getIdKelurahan(hasil2)='') then
                ...
                begin
                  Close;
                  SQL.Clear;
                  SQL.Text:='insert into t_usulsaran (nama,desa_kelurahan,nohp,idtelegram,idchat,pekerjaan,usulan) values ("'+hasil1+'","'+getIdKelurahan(hasil2)+'","-","'+idtelegram+'","'+idchat+'","'+hasil3+'","'+hasil4+'")';
                  ExecSQL;
                end;
                // Usul Saran Tersimpan
                kirimChat(idchat,idtelegram,getBalasan(11));
                end;
            end;  // End Usul Saran
         ...
         if (aksi='HASIL') then
            ...
            hasil1   := AnsiUpperCase(copy(pesan,kar_awal+1));
                ...
                kirimChat(idchat,idtelegram,hitungHasil(hasil1));
         ...
         if (aksi='SUARA') then
            ...
            if (hasil1='CAPRES') then
              begin
                ...
                kirimChat(idchat,idtelegram,getBalasan(12))
                else begin
                  // Cek Apakah Saksi Terdaftar
                  if cekSaksi(idtelegram)>=1 then
                  begin
		    // Cek Apakah Nomor
                    if (IsNumber(hasil3)=True) and (IsNumber(hasil4)=True) and (IsNumber(hasil5)=True) and (IsNumber(hasil6)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),hasil2)<=0 then
                      begin
                        if (tglpemilu=tglchat) then
                        begin
                        with DM.ZQSimpan do
                        begin
                          Close;
                          SQL.Clear;
                          SQL.Text:='insert into t_hitungcapres (no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                          ExecSQL;
                        end;
                      // Suara Tersimpan
                      kirimChat(idchat,idtelegram,getBalasan(13));
                      // Suara Sudah Ada
                      end;
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(14))
		    // Bukan Nomor
                    end else
                    kirimChat(idchat,idtelegram,getBalasan(15))
                  // Saksi Tidak Terdaftar
                  end else
                  kirimChat(idchat,idtelegram,getBalasan(16));
                end;
              end;
              ...
              if (hasil1='KADES') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(54))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungkades (keldesa,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdKades(hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(55));
                        // Suara Sudah Ada
                        end;
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(56))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='CAGUB') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(17))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungcagub (provinsi,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdCagub(hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(18));
                        // Suara Sudah Ada
                        end;
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(19))
  		       // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='CABUPKOTA') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(20))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungcabup (kabkota,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdCabup(hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(21));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(22))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='PARTAI') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(23))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungpartai (no_urut,tps,suara_dprri,suara_dprdprov,suara_dprdkab,suara_sah,suara_tidaksah,dpt) values ("'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'","'+hasil7+'","'+hasil8+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(24));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(25))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='DPD') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(26))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdpdri (dapil,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(27));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(28))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='DPR') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(29))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprri (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(30));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(31))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='DPRDPROV') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(32))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprdprov (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(33));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(34))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
              ...
              if (hasil1='DPRDKABKOTA') then
                begin
                  ...
                  if cekCalon(hasil1,hasil2)<=0 then
                  kirimChat(idchat,idtelegram,getBalasan(35))
                  else begin
                    // Cek Apakah Saksi Terdaftar
                    if cekSaksi(idtelegram)>=1 then
                    begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprdkab (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,hasil2)+'","'+hasil2+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil3+'","'+hasil4+'","'+hasil5+'","'+hasil6+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(36));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(37))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
         ...
         if (aksi='UBAH') then
		    ...
		    // Cek Apakah Nomor
                    if (IsNumber(hasil3)=True) and (IsNumber(hasil4)=True) and (IsNumber(hasil5)=True) and (IsNumber(hasil6)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),hasil2)>=1 then
                      begin
                        if (tglpemilu=tglchat) then
                        begin
                        with DM.ZQSimpan do
                        begin
                          Close;
                          SQL.Clear;
                          SQL.Text:='update t_hitungcapres set no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                          ExecSQL;
                        end;
                      // Suara Tersimpan
                      kirimChat(idchat,idtelegram,getBalasan(38));
                      // Suara Sudah Ada
                      end;
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(39))
		    // Bukan Nomor
                    end else
                    kirimChat(idchat,idtelegram,getBalasan(15))
                  // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungkades set keldesa="'+getIdKades(hasil2)+'",no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(57));
                        // Suara Sudah Ada
                        end;
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(58))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungcagub set provinsi="'+getIdCagub(hasil2)+'",no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(40));
                        // Suara Sudah Ada
                        end;
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(41))
  		       // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungcabup set kabkota="'+getIdCabup(hasil2)+'",no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(42));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(43))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungpartai set no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",suara_dprri="'+hasil3+'",suara_dprdprov="'+hasil4+'",suara_dprdkab="'+hasil5+'",suara_sah="'+hasil6+'",suara_tidaksah="'+hasil7+'",dpt="'+hasil8+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(44));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(45))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungdpdri set dapil="'+getIdDapil(hasil1,hasil2)+'",no_urut="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(46));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(47))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungdprri set dapil="'+getIdDapil(hasil1,hasil2)+'",id_caleg="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(48));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(49))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungdprdprov set dapil="'+getIdDapil(hasil1,hasil2)+'",id_caleg="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(50));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(51))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='update t_hitungdprdkab set dapil="'+getIdDapil(hasil1,hasil2)+'",id_caleg="'+hasil2+'",tps="'+getIdTPSSaksi(idtelegram)+'",perolehan="'+hasil3+'",suara_sah="'+hasil4+'",suara_tidaksah="'+hasil5+'",dpt="'+hasil6+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),hasil2)+'"';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(52));
                        end;
                        // Suara Sudah Ada
                        end else
                        kirimChat(idchat,idtelegram,getBalasan(53))
  		    // Bukan Nomor
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(15))
                    // Saksi Tidak Terdaftar
         ...
         if (aksi='INPUT') then
            ...
            hasil1   := AnsiUpperCase(copy(pesan,kar_awal+1,kar_akhir-kar_awal-1));
      		  ...
      		  // Cek Apakah Nomor
                  if (IsNumber(hasil)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQCari5.FieldByName('no_urut').AsString)<=0 then
                      begin
                      if (tglpemilu=tglchat) then
                      begin
                     // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil6))then
                    //  begin
      		      with DM.ZQSimpan do
      		      begin
      		      Close;
      		      SQL.Clear;
      		      SQL.Text:='insert into t_hitungdpdri (dapil,no_urut,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,DM.ZQCari5.FieldByName('no_urut').AsString)+'","'+DM.ZQCari5.FieldByName('no_urut').AsString+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil+'","'+ssah+'","'+stsah+'","'+dpt+'")';
      		      ExecSQL;
      		      end;
                      // Suara Tersimpan
                      kirimChat(idchat,idtelegram,getBalasan(27));
                     // end else
                    //  kirimChat(idchat,idtelegram,getBalasan(59));
                      end;
                      // Suara Sudah Ada
                      end else
                      kirimChat(idchat,idtelegram,getBalasan(28))
		    ...
		    // Cek Apakah Nomor
                      if (IsNumber(hasil)=True) then
                      begin
                        if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQCari5.FieldByName('id').AsString)<=0 then
                        begin
                        if (tglpemilu=tglchat) then
                        begin
                       // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil6))then
                      //  begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprri (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,DM.ZQCari5.FieldByName('id').AsString)+'","'+DM.ZQCari5.FieldByName('id').AsString+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil+'","'+ssah+'","'+stsah+'","'+dpt+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(30));
  		    ...
  		    // Cek Apakah Nomor
                     ...
                     //   begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprdprov (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,DM.ZQCari5.FieldByName('id').AsString)+'","'+DM.ZQCari5.FieldByName('id').AsString+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil+'","'+ssah+'","'+stsah+'","'+dpt+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(33));
		    ...
		    // Cek Apakah Nomor
		     ...
		     //    begin
                          ...
                          begin
                            Close;
                            SQL.Clear;
                            SQL.Text:='insert into t_hitungdprdkab (dapil,id_caleg,tps,perolehan,suara_sah,suara_tidaksah,dpt) values ("'+getIdDapil(hasil1,DM.ZQCari5.FieldByName('id').AsString)+'","'+DM.ZQCari5.FieldByName('id').AsString+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil+'","'+ssah+'","'+stsah+'","'+dpt+'")';
                            ExecSQL;
                          end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(36));
      		  ...
      		  // Cek Apakah Nomor
                  if (IsNumber(hasil)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQPartai.FieldByName('no_urut').AsString)<=0 then
                      begin
                      if (tglpemilu=tglchat) then
                      begin
                     // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil9))then
                     // begin
                        with DM.ZQSimpan do
                        begin
                          Close;
                          ...
                          SQL.Text:='insert into t_hitungpartai (no_urut,tps,suara_dprdkab,suara_dprdprov,suara_dprri,suara_sah,suara_tidaksah,dpt) values ("'+DM.ZQPartai.FieldByName('no_urut').AsString+'","'+getIdTPSSaksi(idtelegram)+'","0","0","'+hasil+'","'+ssah+'","'+stsah+'","'+dpt+'")';
                          if FUtama.LKategori.Caption='DPRD Provinsi' then
                          SQL.Text:='insert into t_hitungpartai (no_urut,tps,suara_dprdkab,suara_dprdprov,suara_dprri,suara_sah,suara_tidaksah,dpt) values ("'+DM.ZQPartai.FieldByName('no_urut').AsString+'","'+getIdTPSSaksi(idtelegram)+'","0","'+hasil+'","0","'+ssah+'","'+stsah+'","'+dpt+'")';
                          if FUtama.LKategori.Caption='DPRD Kabupaten/Kota' then
                          SQL.Text:='insert into t_hitungpartai (no_urut,tps,suara_dprdkab,suara_dprdprov,suara_dprri,suara_sah,suara_tidaksah,dpt) values ("'+DM.ZQPartai.FieldByName('no_urut').AsString+'","'+getIdTPSSaksi(idtelegram)+'","'+hasil+'","0","0","'+ssah+'","'+stsah+'","'+dpt+'")';
                          ExecSQL;
                        end;
                      // Suara Tersimpan
                      kirimChat(idchat,idtelegram,getBalasan(24));
         ...
         if (aksi='UPDATE') then
            ...
            hasil1   := AnsiUpperCase(copy(pesan,kar_awal+1,kar_akhir-kar_awal-1));
               ...
               //count := DM.ZQDPDRI.RecordCount;
      		  ...
      		  // Cek Apakah Nomor
                  if (IsNumber(hasil)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQCari5.FieldByName('no_urut').AsString)>=1 then
                      begin
                      if (tglpemilu=tglchat) then
                      begin
                     // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil6))then
		    ...
		    // Cek Apakah Nomor
                      if (IsNumber(hasil)=True) then
                      begin
                        if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQCari5.FieldByName('id').AsString)>=1 then
                        begin
                        if (tglpemilu=tglchat) then
                        begin
                       // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil6))then
		    ...
		    // Cek Apakah Nomor
		     ...
		     //    begin
      		  ...
      		  // Cek Apakah Nomor
                  if (IsNumber(hasil)=True) then
                    begin
                      if cekSuara(hasil1,getIdTPSSaksi(idtelegram),DM.ZQPartai.FieldByName('no_urut').AsString)>=1 then
                      begin
                      if (tglpemilu=tglchat) then
                      begin
                     // if (getTotDPT(getIdTPSSaksi(idtelegram))=Trim(hasil9))then
                     // begin
                        ...
                        begin
                          ...
                          SQL.Clear;
                          if FUtama.LKategori.Caption='DPR RI' then
                          SQL.Text:='update t_hitungpartai set no_urut="'+DM.ZQPartai.FieldByName('no_urut').AsString+'",tps="'+getIdTPSSaksi(idtelegram)+'",suara_dprdkab="0",suara_dprdprov="0",suara_dprri="'+hasil+'",suara_sah="'+ssah+'",suara_tidaksah="'+stsah+'",dpt="'+dpt+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),DM.ZQPartai.FieldByName('no_urut').AsString)+'"';
                          if FUtama.LKategori.Caption='DPRD Provinsi' then
                          SQL.Text:='update t_hitungpartai set no_urut="'+DM.ZQPartai.FieldByName('no_urut').AsString+'",tps="'+getIdTPSSaksi(idtelegram)+'",suara_dprdkab="0",suara_dprdprov="'+hasil+'",suara_dprri="0",suara_sah="'+ssah+'",suara_tidaksah="'+stsah+'",dpt="'+dpt+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),DM.ZQPartai.FieldByName('no_urut').AsString)+'"';
                          if FUtama.LKategori.Caption='DPRD Kabupaten/Kota' then
                          SQL.Text:='update t_hitungpartai set no_urut="'+DM.ZQPartai.FieldByName('no_urut').AsString+'",tps="'+getIdTPSSaksi(idtelegram)+'",suara_dprdkab="'+hasil+'",suara_dprdprov="0",suara_dprri="0",suara_sah="'+ssah+'",suara_tidaksah="'+stsah+'",dpt="'+dpt+'" where id="'+getIdHitung(hasil1,getIdTPSSaksi(idtelegram),DM.ZQPartai.FieldByName('no_urut').AsString)+'"';
                          ExecSQL;
                        end;
                        // Suara Tersimpan
                        kirimChat(idchat,idtelegram,getBalasan(44));
     ...
     Close;
     SQL.Clear;
     SQL.Text:='update t_inboxchat set processed="True" where id="'+DM.ZQCariChat.FieldByName('id').AsString+'"';
     ExecSQL;
     end;
     //Next;
     //end; // End Looping
...
begin
  Result:=0;
   ...
   begin
    Close;
    SQL.Clear;
    SQL.Text:='select * from t_usulantimses where idtelegram="'+idtelegram+'"';
    Open;
   end;
...
begin
 Result:=0;
  ...
  Close;
  SQL.Clear;
  SQL.Text:='select * from t_polling where idtelegram="'+idtelegram+'"';
  Open;
  end;
  Result:=DM.ZQTimerCari.RecordCount;
end;
...
Result:=0;
 ...
 begin
  Close;
  SQL.Clear;
  SQL.Text:='select * from t_saksi where id_telegram="'+idtelegram+'"';
  Open;
 end;
 Result:=DM.ZQTimerCari.RecordCount;
end;
...
begin
  ...
  Close;
  SQL.Clear;
  SQL.Text:='select * from t_timses where id_telegram="'+idtelegram+'"';
  Open;
  end;
  Result:=DM.ZQTimerCari.RecordCount;
end;
...
begin
  ...
  Close;
  SQL.Clear;
  SQL.Text:='insert into t_outboxchat (tanggal,jam,isipesan,id_chat,id_tujuan,status) values ("'+FormatDateTime('yyyy-mm-dd',Now)+'","'+FormatDateTime('hh:mm:ss',Now)+'","'+isi+'","'+idchat+'","'+idtelegram+'","Pending")';
  ExecSQL;
  end;
end;
...
Result:='';
 ...
 Close;
 SQL.Clear;
 SQL.Text:='select tps from t_saksi where id_telegram="'+idtelegram+'"';
 Open;
 end;
