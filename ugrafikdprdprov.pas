...
var
  FGrafikDPRDProv: TFGrafikDPRDProv;
  jml : integer;
...
begin
 Result := 'SELECT sum('+Total+') as jml FROM '+
           '((((((t_hitungdprdprov INNER JOIN t_calegdprdprov ON t_calegdprdprov.id=t_hitungdprdprov.id_caleg) '+
...
procedure TFGrafikDPRDProv.BTutupClick(Sender: TObject);
begin
...
procedure TFGrafikDPRDProv.BRefreshClick(Sender: TObject);
begin
  HapusGrafik;
  BuatGrafik;
  Persentase;
...
procedure TFGrafikDPRDProv.BCetakClick(Sender: TObject);
var
  namaimg,titel : string;
begin
  namaimg:='gdprdprovinsi.bmp';
  titel  :='Grafik Perhitungan DPRD Provinsi';
...
procedure TFGrafikDPRDProv.BFilterClick(Sender: TObject);
begin
  HapusGrafik;
  ...
  jml := DM.ZQGDPRDP.RecordCount;
  BuatGrafik;
  Persentase;
end;

procedure TFGrafikDPRDProv.CkAutoRefreshChange(Sender: TObject);
begin
...
procedure TFGrafikDPRDProv.FormCreate(Sender: TObject);
begin
...
procedure TFGrafikDPRDProv.FormResize(Sender: TObject);
begin
  UkuranPanel;
end;

procedure TFGrafikDPRDProv.FormShow(Sender: TObject);
begin
    jml := DM.ZQGDPRDP.RecordCount;
end;

procedure TFGrafikDPRDProv.GrafikCalegAfterDraw(ASender: TChart;
...
var
  ...
  jpg: TJpegImage;
begin
    if (DM.ZQGDPRDP.RecordCount>0) then begin
  DM.ZQGDPRDP.First;
  ...
  jpg := TJpegImage.Create;
    ...
    if FileExists(ExtractFilePath(Application.ExeName)+'img\caleg\dprdprov\'+DM.ZQGDPRDP.FieldByName('foto').AsString) then
    jpg.LoadfromFile(ExtractFilePath(Application.ExeName)+'img\caleg\dprdprov\'+DM.ZQGDPRDP.FieldByName('foto').AsString) else
...
procedure TFGrafikDPRDProv.TimerRefreshTimer(Sender: TObject);
begin
  HapusGrafik;
  BuatGrafik;
  Persentase;
...
procedure TFGrafikDPRDProv.BuatGrafik;
var
i : integer;
persen : Real;
begin
  if (DM.ZQGDPRDP.RecordCount>0) then begin
  DM.ZQGDPRDP.First;
  for i := 0 to DM.ZQGDPRDP.RecordCount-1 do
  begin
  with DM.ZQGrafikDPRProv do
  begin
  Close;
  SQL.Clear;
  SQL.Text:=BuatQuery('t_hitungdprdprov.perolehan')+' where t_calegdprdprov.id="'+DM.ZQGDPRDP.FieldByName('id').AsString+'"';
  Open;
  end;
  if (NilaiUnik('t_hitungdprdprov','suara_sah') > 0) and (DM.ZQGrafikDPRProv.FieldByName('jml').AsInteger > 0) then
  persen := StrToFloat(FormatFloat('0.##',(DM.ZQGrafikDPRProv.FieldByName('jml').AsInteger/NilaiUnik('t_hitungdprdprov','suara_sah'))*100)) else
  persen := 0;
  Bars.AddXY(DM.ZQGDPRDP.FieldByName('id').AsInteger,
             persen,
             DM.ZQGDPRDP.FieldByName('nama_caleg').AsString+sLineBreak+'('+
             FormatCurr('#,###',DM.ZQGrafikDPRProv.FieldByName('jml').AsCurrency)
             +')',
             StringToColor(DM.ZQGDPRDP.FieldByName('warna').AsString)
             );
  DM.ZQGDPRDP.Next;
  end;
...
procedure TFGrafikDPRDProv.HapusGrafik;
var
i : integer;
begin
...
procedure TFGrafikDPRDProv.Persentase;
var
...
begin
  ...
  begin
  // Suara Sah
  PSuaraSah.Caption:='Suara Sah : '+FormatCurr('#,###',NilaiUnik('t_hitungdprdprov','suara_sah'));
  // Suara Tidak Sah
  PSuaraTidakSah.Caption:='Suara Tidak Sah : '+FormatCurr('#,###',NilaiUnik('t_hitungdprdprov','suara_tidaksah'));
  // TPS Masuk
  Close;
  SQL.Clear;
  SQL.Text:='select distinct(tps) as jml from t_hitungdprdprov';
  Open;
  PTPSMasuk.Caption:='TPS Masuk : '+FormatCurr('#,###',RecordCount);
  // Suara Masuk
  PSuaraMasuk.Caption:='Suara Masuk : '+FormatCurr('#,###',NilaiUnik('t_hitungdprdprov','dpt'));
...
procedure TFGrafikDPRDProv.UkuranPanel;
begin
...
function TFGrafikDPRDProv.NilaiUnik(NamaTabel: String; NamaField: String
  ): Integer;
var
  i,hasil : LongInt;
begin
